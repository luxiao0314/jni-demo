...
Java_com_aihuishou_aes_util_AES4CUtil_string2Base64(JNIEnv *env, jclass type,
                                                            jbyteArray buf_) {
    char *str = NULL;
...
Java_com_aihuishou_aes_util_AES4CUtil_base642Byte(JNIEnv *env, jclass type,
                                                          jstring out_str) {
...
static uint8_t AES_KEY[] = "abcdefgabcdefg13";
extern "C"
JNIEXPORT jstring JNICALL
Java_com_aihuishou_aes_util_AES4CUtil_encrypt(JNIEnv *env, jclass type, jstring src_) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_aihuishou_aes_util_AES4CUtil_decrypt(JNIEnv *env, jclass type, jstring encrypted_) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_aihuishou_aes_util_AES4CUtil_getAESKey(JNIEnv *env, jclass type) {
...
Java_com_aihuishou_aes_util_AES4CUtil_setAESKey(JNIEnv *env, jclass type, jstring key_) {
    auto *_key = (unsigned char *) env->GetStringUTFChars(key_, 0);
    //获取key的长度
    auto size = (size_t) env->GetStringUTFLength(key_);
    setKey(_key, size);
